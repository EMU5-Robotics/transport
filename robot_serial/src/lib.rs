...
// for some reason packets larger then this don't work properly
pub const MAX_PKT_SIZE: usize = 500;
        ...
        while self.port.bytes_to_read()? != 0 {
            if let Ok(read) = self.port.read(&mut self.partial_read_buffer) {
                ...
                if let Some(idx) = self.read_buffer.iter().position(|&v| v == 0) {
                    let mut tbuf = self.read_buffer.drain(..=idx).collect::<Vec<_>>();
                    let Ok(v) = postcard::from_bytes_cobs::<protocol::ToRobot>(&mut tbuf) else {
                        log::warn!("Parse error: {tbuf:?}");
                        continue;
                    };
